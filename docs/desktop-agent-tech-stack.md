...
| 包含 | 明确不包含 |
|---|---|
| 单工作区、会话列表/新建/打开/fork/重命名/置顶/可恢复删除、跨项目标题搜索 | 内置终端、文件树、Git 面板 |
| 文本/推理/工具事件、流式 Markdown、文件 diff | MCP、附件、多工作区并行 |
| provider 选择与认证、模型选择、停止、基础错误恢复、运行中待发送队列（查看/编辑/取消） | 项目内 extensions/skills 自动加载 |
| 工具审批、项目 Trust、审计日志 | 对话正文全文搜索、永久 shell 允许规则、自动更新 |

运行中待发送队列的 IPC、持久化及停止语义见 [消息队列](prompt-queue)。
...
### 5.4 对话查找与置顶（2026-09-05）

- 侧边栏「搜索对话」或 ⌘/Ctrl+Shift+F 打开弹窗，按标题和项目名过滤所有已添加项目的对话，默认按修改时间倒序；不受项目折叠及分页影响。支持上下键、回车、Esc、点击遮罩关闭。
- 会话菜单提供带图标的重命名、置顶/取消置顶。置顶条目集中展示在项目列表上方，原项目列表不重复显示。
- 置顶以应用自有会话文件路径作为标识，保存在 renderer 的本地 UI 偏好中，刷新或重启保留；已移除项目及已删除对话不会显示在置顶区。搜索只使用现有项目会话目录的标题元数据，不读取正文，不增加 IPC 或改变权限模型。
